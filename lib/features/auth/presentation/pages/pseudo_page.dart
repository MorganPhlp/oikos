...
class PseudoPage extends StatefulWidget {
  ...
  static MaterialPageRoute<dynamic> route({
    ...
    builder: (context) => PseudoPage(email: email, password: password),
  );

  const PseudoPage({super.key, required this.email, required this.password});
...
class _PseudoPageState extends State<PseudoPage> {
  ...
  void _submit() {
    if (_formKey.currentState!.validate()) {
      context.read<AuthBloc>().add(
        AuthValidatePseudo(pseudo: _pseudoController.text.trim()),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // Pas de background color hardcodé
      appBar: AppBar(
        leading: IconButton(
          // L'icône prend la couleur du texte par défaut (Noir en light, Blanc en dark)
          icon: Icon(
            Icons.chevron_left,
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            // Vert adaptatif (Primary avec transparence)
                          ...
                          child: Text(
                            ...
                            style: TextStyle(
                              // On garde une bonne lisibilité selon le fond
                              color: colorScheme.onSurface.withValues(
                                alpha: 0.8,
                              ),
                            ),
