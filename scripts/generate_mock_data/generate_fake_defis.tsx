...
export async function generateFakeDefis(userIds: string[], supabase: SupabaseClient) {
    console.log("🏆 Génération des défis 'Seuil Critique' pour Chloé Robert...");

    const categories = ['Alimentation', 'Transport', 'Logement', 'Numérique', 'Divers'];
    ...
    const { data: memberStats } = await supabase.from('vue_communaute_stats').select('code, nb_membres');
    ...
    for (let i = 0; i < communautes.length; i++) {
        ...
        const { data: defiVote } = await supabase.from('defi').insert({
        ...
        if (defiVote) {
            const quorum1 = Math.ceil((memberStats?.find(m => m.code === c1)?.nb_membres || 0) * 0.6);
            const quorum2 = Math.ceil((memberStats?.find(m => m.code === c2)?.nb_membres || 0) * 0.6);
            ...
            console.log(`  🗳️ Défi Vote créé : ${c1}(${n1}/${quorum1}) vs ${c2}(${n2}/${quorum2})`);
        }
        ...
        if (estDansCommu1 || estDansCommu2) {
            const { data: action } = await supabase.from('actions').select('id').limit(1).single();

            const { data: defiActif } = await supabase.from('defi').insert({
                titre_personnalise: `Action Finale ${c1} vs ${c2}`,
            ...
            if (defiActif) {
                ...
                const membersCount = memberStats?.find(m => m.code === targetCommu)?.nb_membres || 0;
                const threshold = Math.ceil(membersCount * 0.6);
                const nParticipation = threshold - 1;

                await fillParticipation(defiActif.id, otherUserIds, targetCommu, nParticipation, supabase);
                console.log(`  🔥 Défi Actif créé : Chloé doit être la ${threshold}ème personne de ${targetCommu}`);
            }
        }
    }
}

async function fillVotes(defiId: string, userIds: string[], c1: string, n1: number, c2: string, n2: number, supabase: SupabaseClient) {
    const { data: users } = await supabase.from('utilisateur').select('id, code_communaute').in('id', userIds);
    if (!users) return;

    const votes = [];
    const participantsC1 = users.filter(u => u.code_communaute === c1).slice(0, n1);
    const participantsC2 = users.filter(u => u.code_communaute === c2).slice(0, n2);

    for (const u of [...participantsC1, ...participantsC2]) {
        votes.push({ defi_id: defiId, user_id: u.id, est_favorable: true });
    }
    if (votes.length > 0) await supabase.from('votes_lancement_defi').insert(votes);
}

async function fillParticipation(defiId: string, userIds: string[], commu: string, n: number, supabase: SupabaseClient) {
    const { data: users } = await supabase.from('utilisateur').select('id').eq('code_communaute', commu).in('id', userIds);
    if (!users) return;
    ...
    if (participations.length > 0) await supabase.from('defi_participation').insert(participations);
}