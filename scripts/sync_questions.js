...
const TARGET_QUESTIONS = [
  ...
  [
    "divers . numérique . appareils . renouvellement téléphone",
    "📱",
    "optionnelle",
  ],
...
function determineWidgetType(rule) {
  ...
  if (
    raw.plancher !== undefined ||
    raw.plafond !== undefined ||
    raw.unité !== undefined
  ) {
    return "NOMBRE";
  }
...
function buildConfigJson(slug, rule, widgetType) {
  ...
  if (raw.mosaique && raw.mosaique.options)
    config.options = raw.mosaique.options;
